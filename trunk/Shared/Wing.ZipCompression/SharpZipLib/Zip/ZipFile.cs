...
namespace ICSharpCode.SharpZipLib.Zip 
{
	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		public int FindEntry(string name, bool ignoreCase)
		{
            ...
            CompareOptions options = ignoreCase ? CompareOptions.None : CompareOptions.IgnoreCase;

			// TODO: This will be slow as the next ice age for huge archives!
			for (int i = 0; i < entries_.Length; i++) {
                if (string.Compare(name, entries_[i].Name, CultureInfo.InvariantCulture, options) == 0)
                {
					return i;
				}
			}
...
//		}
		...
		void AddUpdate(ZipUpdate update)
		{
			...
			if (index >= 0) {
				...
				updateCount_ += 1;
				updateIndex_.Add(update.Entry.Name);
			}
		}
...
*/
		...
		int FindExistingUpdate(ZipEntry entry)
		{
			...
			if (updateIndex_.Contains(convertedName)) {
				result = (int)updateIndex_.IndexOf(convertedName);
			}
/*
 ...
 */
		...
		/// </summary>
		class UpdateComparer : System.Collections.Generic.IComparer<ZipUpdate>
		{
			/// <summary>
            ...
            #region IComparer<ZipUpdate> Members
