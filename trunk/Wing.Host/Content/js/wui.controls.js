...
(function WuiUI(ctx)
{
    ...
    Type.define("wui.CommandDescriptor", null, {
        fields:
        {
            isUiCommand: true
        },
        ...
        properties: {
            id: { supressBinding: true },
            text: { supressBinding: true },
            iconClass: { supressBinding: true },
            order: { supressBinding: true },
            hint: { supressBinding: true },
            group: { supressBinding: true },
            execute: { supressBinding: true }
        }
    });

    Type.define("wui.CommandManager", null, {
        methods: {
            initialize: function ()
            {
                this.base();
            ...
            {
                options.id = name;
                var cmd = this._commands[name] = new wui.CommandDescriptor(options);
                this._list.push(cmd);
            },
            getCommand: function (name)
            {
                ...
                if (name.isUiCommand)
                    return name;
                var result = this._commands[name];
                if (!result)
                {
                    var parentMngr = util.getFromParentWindow("Command");
                    if (parentMngr)
                        result = parentMngr.getCommand(name);
                }
                return result;
            },
            getGroupCommands: function (groupName)
            {
                return Enum(this._list)
    ...
    ctx.Command = new wui.CommandManager();
    ...
    ctx.DATA_WUI_TYPE_FIRST = "[data-wui-type]:first";
    ctx.DATA_WUI_TYPE = "[data-wui-type]";
    ...
    Type.define("wui.Control", null, {
        ...
        properties: {
            ...
            hint: {
                get: function () { return this.el.attr("title"); },
                set: function (value) { this.el.attr("title", value || ""); }
            }
        }
    });
    ...
    Type.define("wui.Container", "wui.Control", {
        ...
        methods: {
            ...
            {
                this._childrenHolder.append(item.el);
            },
            ...
            {
                return util.getArray(this._childrenHolder.children(DATA_WUI_TYPE).wuiControl());
            },
            getChild: function (id)
            {
                if (!id) return undefined;
                else if (id.isUIControl) return id;
                else if (typeof id === "string") return this._childrenHolder.children("#" + id + DATA_WUI_TYPE_FIRST).wuiControl();
                else if (util.isArray(id)) return Enum(this.getChildren()).first(id);
                return this.getChildren()[id];
            },
            getControl: function (id)
            {
                return this.el.find("#" + id + DATA_WUI_TYPE_FIRST).wuiControl();
            },
            dataBind: function (data)
            {
                this.base(data);
                Enum(this.getChildren()).each(function (child) { child.dataBind(); });
            }
        }
    });
        ...
        methods: {
            ...
            updateUi: function ()
            {
                ...
                for (var i = 0, child; (child = children[i]); i++)
                {
                    ...
                    if (p.currGridRow !== p.gridRow || p.currGridCol !== p.gridCol)
                    {
                        if (p.currGridRow > -1)
                            child.el.detach();
    ...
    Type.define("wui.WindowManager", "wui.Container", {
        ...
        methods: {
            ...
            showContextMenu: function (menu, relativeTo)
            {
                if (!menu.ctxWnd)
                {
                    menu.ctxWnd = this;
                    relativeTo = relativeTo ? (relativeTo.isUiControl ? relativeTo.el : $(relativeTo)) : null;
                    if (relativeTo)
                        menu._relativeTo = DOMHelper.getMetrics(relativeTo);
                }
                var parent = util.getFromParentWindow("WindowManager");
                if (parent)
                    parent.showContextMenu(menu);
                else
                {
                    if (this._contextWindowVisible)
                        this._checkContextMenuState();
                    menu.__wndMngrBeginShow();
                    ...
                    for (var i = 0, mItem; (mItem = menu._items[i]); i++)
                    {
                        var item = items.newTag("li", { class: "ui-state-default wui-context-menu-item" });
                        item.attr("data-ctxmenu-item-key", mItem._uid);
                    ...
                    this._currentCtxMenu = menu;
                    this._showContextWindow(elements, relativeTo);
                }
            },
            _showContextWindow: function (content, relativeTo)
            {
                // reset element
            ...
            {
                var options = this._currentCtxMenu;
                if (!options) return;
                var el = this._contextWindow;
                if (options._relativeTo)
                {
                    var winMetrics = DOMHelper.getMetrics(this.el);
                    var elMetrics = DOMHelper.getMetrics(el);
                    var relMetrics = DOMHelper.getMetrics(options._relativeTo);
            ...
            _checkContextMenuState: function ()
            {
                if (this._contextWindowVisible)
                {
                    var menu = this._currentCtxMenu;
                    if (menu && menu.__wndMngrClosing)
                        menu.__wndMngrClosing();
            ...
            _notifyItemClicked: function (itemKey)
            {
                try
                {
                    var menu = this._currentCtxMenu;
                    if (!menu) return;
                    var item = Enum(menu._items).first(function (i) { return i._uid == itemKey; });
                    menu.__wndMngrItemClicked(item);
                }
                finally
                {
                    this._checkContextMenuState();
    ...
    $(".wui-context-menu .wui-context-menu-item").live("click", function (e)
    {
        var _this = $(this);
        var itemKey = _this.attr("data-ctxmenu-item-key");
        if (!itemKey)
            return;
        WindowManager._notifyItemClicked(itemKey);
    });
    ...
    Type.define("wui.ContextMenuItem", null, {
        methods: {
            initialize: function (options)
            {
                this._uid = util.uid();
        ...
        properties: {
            ...
            command: {
                ...
                set: function (value)
                {
                    ...
                    if (!value) return;
                    if (typeof value == "string")
                        this._command = Command.getCommand(value);
                    if (!this._command)
                        throw "Command not found: " + value;
                    value = this._command;
    ...
    Type.define("wui.ContextMenu", null, {
        methods: {
            initialize: function ()
            {
                this._showing = false;
            ...
            {
                WindowManager.showContextMenu(this, relativeTo);
            },
            close: function ()
            {
                if (this._showing)
                    WindowManager.closeContextMenu();
            },
            addItem: function (options)
            {
                if (!options) return;
                this._items.push(new wui.ContextMenuItem(options));
            },
            addCommand: function (commandName, commandParam)
            {
                this.addItem({ command: commandName, commandParam: commandParam });
            },
            addCommandGroup: function (commandGroup, commandParam)
            {
                var self = this;
                Enum(Command.getGroupCommands(commandGroup)).each(function (x) { self.addCommand(x, commandParam); });
            },
            clear: function ()
            {
                this._items = [];
            },
            __wndMngrBeginShow: function ()
            {
                if (!this._showing)
                {
                    this._showing = true;
            ...
            __wndMngrItemClicked: function (item)
            {
                this.triggerItemClicked(item);
                if (item.command())
                    item.command().exec(item.commandParam());
            },
            __wndMngrClosing: function ()
            {
                if (this._showing)
                {
                    this._showing = false;
    ...
    Type.define("wui.Button", "wui.Control", {
        ...
        methods: {
            init: function ()
            {
                ...
                this.el.click(function (e)
                {
                    ...
                    if (button._command)
                        Command.execute(button._command, button.commandParam());
                    if (button._dd_group)
                    {
                        button.checkContextMenu();
                        e.stopPropagation();
                    }
            ...
            updateUi: function ()
            {
                if (!util.isEmpty(this._dd_group))
                {
                    this.secondIconClass("ui-icon-carat-1-s");
                    this.isToggle(true);
                }
            ...
            checkContextMenu: function ()
            {
                ...
                {
                    var self = this;
                    var dmenu = this.dropMenu;
                    if (this._isChecked)
                    {
                        if (!dmenu)
                        {
                            dmenu = this.dropMenu = new wui.ContextMenu();
                            dmenu.onClosed(function ()
                            {
                                self._isChecked = false;
                                self.update();
                            });
                            dmenu.onItemClicked(function (item)
                            {
                                self.triggerMenuItemClick(item.id());
                            });
                        }
                        this.dropMenu.clear();
                        this.dropMenu.addCommandGroup(this._dd_group, this.commandParam());
                        this.dropMenu.show(this);
                    }
                    else if (dmenu)
                    {
                        dmenu.close();
        ...
        properties: {
            ...
            dropDownMenuGroup: {
                get: function () { return this._dd_group || ''; },
                set: function (value)
                {
                    this._dd_group = value || '';
                    this.update();
                }
            },
            command: {
            ...
            commandParam: {
                ...
                set: function (value) { this._commandParam = value; }
            }
        },
        events: {
            click: null,
    ...
    Type.define("wui.Dialog", "wui.Container", {
        methods: {
            init: function ()
            {
                this.base();
                var self = this;
                this.el.addClass("wui-dialog-content");
                this._childrenHolder = this.el.newTag("div", { class: "wui-dialog-content-inner" });
                this._buttonsBar = this.el.newTag("div", { class: "ui-widget-content wui-dialog-button-bar" });
                this.el.dialog({ autoOpen: false,
                    closeText: "fechar",
                    dialogClass: "wui-dialog op-box-shadow",
                    zIndex: 500,
                    width: 'auto',
                    height: 'auto',
            ...
            addDialogButton: function (options)
            {
                var button = new wui.Button(options);
                this._buttonsBar.append(button.el);
            }
        },
        properties: {
            title: {
    ...
    var INPUT_CONTROL_CLEAR_ERRORS = "input-control-clear-errors";
    var INPUT_CONTROL_VALIDATE = "input-control-validate";

    Type.define("wui.DataForm", "wui.Container", {
        methods: {
            init: function ()
            {
                this.base();
                this.el.addClass("wui-data-form");
            },
            clearErrors: function ()
            {
                this.notifyChildren(INPUT_CONTROL_CLEAR_ERRORS);
            },
            validate: function ()
            {
                this.notifyChildren(INPUT_CONTROL_VALIDATE);
                return this.el.find(".wui-input-control.has-errors").length == 0;
            },
            getFieldValue: function (id)
            {
                var field = this.el.find("#" + id + ".wui-input-control").wuiControl();
                if (field)
                    return field.value();
                return undefined;
            }
    ...
    wui.Container.extend().method("addDataForm", function (options) { (new wui.DataForm(options)).addTo(this); return this; });
    wui.Container.extend().method("newDataForm", function (options) { return (new wui.DataForm(options)).addTo(this); });

    Type.define("wui.InputControlBase", "wui.Control", {
        methods: {
            init: function (attr, inputTag)
            {
                this.base();
                var self = this;
                this.el.addClass("wui-input-control");
                this.label = this.el.newTag("label");
                var holder = this.el.newTag("span", { class: "input-content" });
                this.input = holder
                    .newTag(inputTag || "input", attr)
                    .addClass("input");
                this.error = holder.newTag("span", { class: "error-message" });
                this.el.newTag("span", { class: "ui-helper-clearfix" });
                this.acceptMessage(INPUT_CONTROL_CLEAR_ERRORS, function (msgId, msgParam)
                {
                    self.clearError();
                });
                this.acceptMessage(INPUT_CONTROL_VALIDATE, function (msgId, msgParam)
                {
                    self.validate();
                });
            },
            ...
            setFocus: function ()
            {
                this.dispatch("setFocus", function () { this.input.focus(); });
            },
            clearError: function ()
            {
                this.errorMessage('');
        ...
        properties: {
            value: {
                get: function () { return this.getInputValue(); },
                set: function (value) { this.setInputValue(value); }
            },
            caption: {
                get: function () { return this.label.text(); },
                set: function (value) { this.label.text(value); }
            },
            readOnly: {
                get: function () { return this.input.attr("readonly") == "readonly"; },
                set: function (value)
                {
                    if (value == true)
            ...
            required: {
                get: function () { return this.el.hasClass("required"); },
                set: function (value)
                {
                    if (value == true)
                    {
            ...
            hintText: {
                get: function () { return this.input.attr('placeholder'); },
                set: function (value) { this.input.attr("placeholder", value); }
            },
            errorMessage: {
                get: function () { return this.error.text(); },
                set: function (value)
                {
    ...
    Type.define("wui.TextBox", "wui.InputControlBase", {
        methods: {
            init: function ()
            {
                this.base({ type: "text" });
                this.el.addClass("wui-textbox");
            }
        },
        properties: {
        }
    ...
    wui.Container.extend().method("addTextBox", function (options) { (new wui.TextBox(options)).addTo(this); return this; });
    wui.Container.extend().method("newTextBox", function (options) { return (new wui.TextBox(options)).addTo(this); });

    Type.define("wui.EmailBox", "wui.InputControlBase", {
        methods: {
            init: function ()
            {
                this.base({ type: "email" });
                this.el.addClass("wui-emailbox");
            }
        },
        properties: {
        }
    ...
    wui.Container.extend().method("addEmailBox", function (options) { (new wui.EmailBox(options)).addTo(this); return this; });
    wui.Container.extend().method("newEmailBox", function (options) { return (new wui.EmailBox(options)).addTo(this); });

    Type.define("wui.PasswordBox", "wui.InputControlBase", {
        methods: {
            init: function ()
            {
                this.base({ type: "password" });
                this.el.addClass("wui-passwordbox");
            }
        },
        properties: {
        }
    ...
    wui.Container.extend().method("addPasswordBox", function (options) { (new wui.PasswordBox(options)).addTo(this); return this; });
    wui.Container.extend().method("newPasswordBox", function (options) { return (new wui.PasswordBox(options)).addTo(this); });


    Type.define("wui.Section", "wui.Control", {
        methods: {
            init: function ()
            {
                this.el.addClass("ui-widget ui-widget-content wui-section");
                this.textEl = this.el.newTag("span", { class: "wui-section-text" });
            }
        },
        properties: {
            caption: {
                get: function () { return this.textEl.text(); },
                set: function (value) { this.textEl.text(value || ''); }
            }
        }
    });

    wui.Container.extend().method("addSection", function (options) { (new wui.Section(options)).addTo(this); return this; });
    wui.Container.extend().method("newSection", function (options) { return (new wui.Section(options)).addTo(this); });
