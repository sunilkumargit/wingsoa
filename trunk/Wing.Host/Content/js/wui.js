            ...
            processBinding: function (binding, customData, auto)
            {
                ...
                else if (typeof src === "string")
                {
                    if (src.startsWith("@")) // starts with @ - is a 
                    {
                        ...
                        value = data;
                    else if (data.isW$Object)
                    {
                        var member = data[binding.source];
                        if (util.isFunction(member))
                            value = member.call(data);
                        else
                            value = member;
                    }
                    else
    ...
    Type.define("wui.component.Loader", null, {
        ...
        methods: {
            ...
            _done: function (res)
            {
                ...
                this.triggerResourceLoaded(res.name);
                if (res.callback)
                    res.callback(res);
                this.dispatchQueue();
            },
            ...
            processQueue: function ()
            {
                ...
                while (item._left.length)
                {
                    ...
                    else if (res.status === this.RES_STATUS_UNLOADED)
                    {
                        ...
                        this.triggerResourceLoading(res.name);
                        var loader = this;
                        if (res.type === "css")
